...
drop trigger if exists tr_aft_ins_dayofmatch//
DELIMITER //
create trigger tr_aft_ins_dayofmatch after insert on dayofmatch
for each row 
begin 
	if new.id_match is not null then
		update matches 
...
drop trigger if exists tr_bef_ins_dayofmatch//
create trigger tr_bef_ins_dayofmatch before insert on dayofmatch
for each row
begin
	if new.id_match is not null and new.busy_time is null then 
		set new.busy_time = 1;
