...
create procedure insPlayerInMatche2(
	in t_home varchar(100),
    in t_guest varchar(100),
    in tour int,
    in name_player varchar(200),
    in t_player varchar(100),
    in goals int,
    in assists int,
    in c_yellow int,
    in c_red int,
    ...
    in l_own_goal int,
    in l_url_player varchar(200),
    in player_team varchar(100),
    in l_amplua varchar(50))
begin
    ...
    if l_id_match is null then 
		SIGNAL SQLSTATE '02000' SET MESSAGE_TEXT = 'Error: Not found match id and match played!!!!!';
    end if;
    #вытаскиваем игрока
    select id_player
     into l_id_player
    ...
    if l_id_player is not null then 
       ...
       #проставляем амплуа
       update players
       set id_amplua = (select id_amplua from amplua where name_amplua = l_amplua)
       where id_player = l_id_player;
	else 
		...
		WHERE TABLE_SCHEMA = 'football_main_ucl'
        ...
        set name = name_player, 
            ...
            id_amplua = (select id_amplua from amplua where name_amplua = l_amplua);
